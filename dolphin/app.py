...
from dolphin.endpoints.systemendpoint import SystemEndpoint

uowm = SqlAlchemyUnitOfWorkManager(get_db_engine())

cors = CORS(allow_origins_list=['http://localhost:8888', 'http://api.hale-terminal.com'], allow_all_methods=True, allow_all_headers=True, log_level='DEBUG')

api = application = falcon.API(middleware=[cors.middleware, MultipartMiddleware()])
api.req_options.auto_parse_form_urlencoded = True

api.add_route('/v0', SystemEndpoint())
api.add_route('/v0/user/{user}', UserEndpoint(uowm), suffix='user')
