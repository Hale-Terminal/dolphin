...
class BuildAssetsCommand(BaseBuildCommand):
    ...
    def _get_package_version(self):
        ...
        try:
            ...
            log.info(f"pulled version information from 'dolphin' module. {dolphin.__file__}")
            version = self.distribution.get_version()
            build = dolphin.__build__
        finally:
            sys.path.pop(0)
    ...
    def _build(self):
        ...
        if not version_info["version"] or not version_info["build"]:
            log.fatal("Could not determine dolphins version or build")
            sys.exit(1)
        ...
        try:
            ...
            traceback.print_exc()
            log.fatal("unable to build Dolphin's static assets!")
            sys.exit(1)
    ...
    def _build_static(self):
        ...
        env = dict(os.environ)
        env["COSMOS_STATIC_DIST_PATH"] = self.cosmos_static_dist_path
    ...
    @property
    def cosmos_static_dist_path(self):
        return os.path.abspath(os.path.join(self.build_lib, "src/static/dolphin/dist"))
...
VERSION = "0.1.0.dev0"
IS_LIGHT_BUILD = os.environ.get("DOLPHIN_LIGHT_BUILD") == "1"
...
class DolphinSDistCommand(SDistCommand):
...
class DolphinBuildCommand(BuildCommand):
    def run(self):
        if not IS_LIGHT_BUILD:
...
class DolphinDevelopCommand(DevelopCommand):
    def run(self):
...
cmdclass = {
    "sdist": DolphinSDistCommand,
    "develop": DolphinDevelopCommand,
    "build": DolphinBuildCommand,
